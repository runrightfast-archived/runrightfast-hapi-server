...
(function() {
	...
	var HapiServer = function HapiServer(options) {
		...
		var isAutoStart = function isAutoStart() {
			if (lodash.isUndefined(options.autoStart)) {
				return true;
			}
		...
		var getName = function getName() {
			if (lodash.isUndefined(options.name)) {
				return 'runrightfast-hapi-server';
			}
			assert(lodash.isString(options.name), 'options.name must be a String');
			return options.name;
		};

		var getStopTimeout = function() {
			if (lodash.isUndefined(options.stopTimeout)) {
				return 60 * 1000;
			}
		...
		this.name = getName();
		this.startCount = 0;
		this.stopTimeout = getStopTimeout();
			...
			this.serverDomain.run(function() {
				var _start = function() {
					self.composer.start(function() {
						...
						log.info('Hapi Server started : ' + info);
						self.emit('STARTED');
						if (callback) {
				...
				if (self.startCount === 0) {
					self.composer.compose(function(err) {
						if (err) {
							log.error('Failed composing Hapi Server : ' + err.message);
							if (callback) {
